...
async def _find_assignment_target(db: AsyncSession, ticket_type: TicketType, client_id: str) -> str | None:
    if ticket_type == TicketType.deliverable_revision:
        # First, try to route to a specific team member assigned to this client
        result = await db.execute(
            select(TeamMember.user_id)
            .join(ClientAssignment, ClientAssignment.team_member_id == TeamMember.id)
            .where(
    ...
    elif ticket_type in (TicketType.general_support, TicketType.billing_issue):
...
async def create_ticket(
    ...
    assigned_to = await _find_assignment_target(db, validated_type, current_user.id)
...
async def resolve_ticket(
    ...
    try:
        ...
        await db.rollback()
        logger.exception("Transaction failed during ticket resolution")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Database transaction failed.",
        )
...
async def reopen_ticket(
    ...
    if ticket.resolved_at is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Ticket has no resolution timestamp. Cannot reopen.",
        )

    now = datetime.now(timezone.utc)
    elapsed = now - ticket.resolved_at

    if elapsed > timedelta(days=REOPEN_WINDOW_DAYS):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=(
                f"The {REOPEN_WINDOW_DAYS}-day reopen window has expired "
                f"(resolved {elapsed.days} days ago). Please create a new ticket."
            ),
        )
    ...
    try:
        ...
        await db.rollback()
        logger.exception("Transaction failed during ticket reopen")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Database transaction failed.",
        )
