    ...
    async with async_session_factory() as db:
        async with tenant_session(db, agency_id=agency_id):
            ...
            notif = Notification(
                ...
                # Note: Notification model might have agency_id. If yes, the session context handles RLS.
                # However, for insert we might need to specify it manually if the DB doesn't set it automatically.
                # Actually, RLS does not automatically set it unless we have a trigger, so we must set it.
                # Wait, if Notification model has agency_id mapped, we should set it if available.
                # Let's set it if agency_id is provided.
            )
            if agency_id:
                notif.agency_id = agency_id
...
@celery_app.task(name="app.workers.tasks.notify.send_notification_task", queue="default")
def send_notification_task(
    ...
    return cast(dict[str, Any], run_async_safe(
