...
export function SlaPerformanceWidget({ slas }: SlaPerformanceWidgetProps) {
  ...
  return (
    <div
      ...
      className="bg-white rounded-2xl border border-slate-200/80 shadow-2xs hover:shadow-md hover:border-blue-200 transition-all p-4 sm:p-5 flex flex-col w-full font-sans cursor-pointer group"
    >
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[9px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100/50 shadow-2xs">
            Goal 98.0%
          </span>
        </div>
        <span className="text-[9px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full shadow-2xs">
          Optimal
        </span>
      </div>
      <p className="text-[11px] text-slate-500 font-medium mb-3">Daily metrics on response & resolution deadlines</p>

      <div className="flex items-center gap-3 sm:gap-4 mb-3.5 bg-slate-50/70 border border-slate-100 rounded-xl p-3 shadow-2xs">
        {/* Circular Progress */}
        <div className="relative w-[64px] h-[64px] flex items-center justify-center flex-shrink-0">
          ...
          <div className="absolute flex flex-col items-center">
            <span className="text-[8px] font-black text-slate-400 tracking-wider">SLA</span>
            <span className="text-[14px] font-black text-slate-900 tracking-tighter leading-none my-0.5">{overallSla}%</span>
            <span className="text-[7px] font-black text-blue-600 tracking-wider">Met</span>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2 flex-1">
          <div className="flex flex-col gap-1">
            ...
            <div className="w-full h-[5px] bg-slate-200/60 rounded-full overflow-hidden">
              <div className="h-full bg-blue-600 rounded-full" style={{ width: `${responseSla}%` }} />
            </div>
          </div>
          <div className="flex flex-col gap-1">
            <div className="flex justify-between text-[11px] font-bold tracking-tight">
      ...
      <div className="flex flex-col">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-1.5">
            ...
            <h3 className="text-xs font-black text-slate-900 tracking-tight">Alerts Raised</h3>
          </div>
          <span className="text-[9px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full shadow-2xs">
            {activeAlerts} Active
          </span>
        ...
        <div className="flex flex-col gap-2">
          {slas.length > 0 ? (
            slas.slice(0, 2).map((sla) => (
              <div key={sla.id} className="flex items-center justify-between p-2.5 rounded-xl border border-rose-100 bg-rose-50/30">
            ...
            <>
              ...
              <div className="flex items-center justify-between p-2.5 rounded-xl border border-slate-100 bg-white">
                ...
                <span className="text-[9px] font-black text-blue-600 ml-2 tracking-wide shrink-0">Cleared</span>
              </div>
            </>
          )}
        </div>
      ...
      <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-400 font-medium" onClick={(e) => e.stopPropagation()}>
